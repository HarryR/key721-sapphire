...
// https://github.com/OpenZeppelin/openzeppelin-contracts/blob/8d633cb7d169f2f8595b273660b00b69e845c2fe/test/token/ERC721/ERC721.behavior.js
...
function shouldBehaveLikeERC721(errorPrefix, owner, newOwner, approved, anotherApproved, operator, other) {
  ...
  context('with minted tokens', function () {
    ...
    describe('balanceOf', function () {
      ...
      context('when querying the zero address', function () {
        it('throws', async function () {
          await expectRevertCustomError(this.token.balanceOf(ZERO_ADDRESS), 'Error_ERC721_Address_Zero_Is_Not_A_Valid_Owner()');
        });
      });
    });
    ...
    describe('ownerOf', function () {
      ...
      context('when the given token ID was not tracked by this token', function () {
        ...
        it('reverts', async function () {
          await expectRevertCustomError(this.token.ownerOf(tokenId), 'Error_ERC721_Invalid_Token_ID');
        });
      });
    });
        ...
        context('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await expectRevertCustomError(
              ...
              'Error_ERC721_Transfer_To_The_Zero_Address()',
            );
          });
        });
      };
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            ...
            await expectRevertCustomError(
              ...
              'Error_ERC721_Transfer_To_Non_ERC721Receiver_implementer()',
            );
          });
        });
    ...
    describe('approve', function () {
      ...
      context('when the address that receives the approval is the owner', function () {
        it('reverts', async function () {
          await expectRevertCustomError(this.token.approve(owner, tokenId, { from: owner }), 'Error_ERC721_Approval_To_Current_Owner()');
        });
      });
    ...
    describe('setApprovalForAll', function () {
      ...
      context('when the operator is the owner', function () {
        it('reverts', async function () {
          await expectRevertCustomError(this.token.setApprovalForAll(owner, true, { from: owner }), 'Error_ERC721_Approve_To_Caller()');
        });
      });
    });
    ...
    describe('getApproved', async function () {
      context('when token is not minted', async function () {
        it('reverts', async function () {
          await expectRevertCustomError(this.token.getApproved(nonExistentTokenId), 'Error_ERC721_Invalid_Token_ID()');
        });
      });
  ...
  describe('_mint(address, uint256)', function () {
    it('reverts with a null destination address', async function () {
      await expectRevertCustomError(this.token.mint(ZERO_ADDRESS, firstTokenId), 'Error_ERC721_Mint_To_The_Zero_Address()');
    });
    ...
    context('with minted token', async function () {
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expectRevertCustomError(this.token.mint(owner, firstTokenId), 'Error_ERC721_Token_Already_Minted()');
      });
    });
  });
  ...
  describe('_burn', function () {
    it('reverts when burning a non-existent token id', async function () {
      await expectRevertCustomError(this.token.burn(nonExistentTokenId), 'Error_ERC721_Invalid_Token_ID()');
    });
    ...
    context('with minted tokens', function () {
      ...
      context('with burnt token', function () {
        ...
        it('deletes the token', async function () {
          expect(await this.token.balanceOf(owner)).to.be.bignumber.equal('1');
          await expectRevertCustomError(this.token.ownerOf(firstTokenId), 'Error_ERC721_Invalid_Token_ID()');
        });

        it('reverts when burning a token id that has been deleted', async function () {
          await expectRevertCustomError(this.token.burn(firstTokenId), 'Error_ERC721_Invalid_Token_ID()');
        });
      });
    });
  });
  ...
  describe('metadata', function () {
    ...
    describe('token URI', function () {
      ...
      it('reverts when queried for non existent token id', async function () {
        await expectRevertCustomError(this.token.tokenURI(nonExistentTokenId), 'Error_ERC721_Invalid_Token_ID');
      });
